...
### DayKey — how a "day" is identified (READ THIS BEFORE TOUCHING ANY DAY LOOKUP)
- Location: `Core/Models/DayKey.swift`. A calendar day is an **`Int` (`yyyymmdd`)**, stored as `dayKey` on `DailyPage` and `CalendarEventLocalState`.
- **Never look a day record up by matching its `date`.** `date` is an *instant* (local midnight when the row was written). Matching instants only works while the device never leaves that timezone. The owner travelled, the timezone changed, and **every** lookup missed: the app couldn't see pages it already had, generated fresh empty ones over real days, and completed past days came back unchecked and broke streaks. Look up by `dayKey`; compare days with `DayKey.make(...)`, not with `<`/`==` on Dates.
- `DayKey.fromStoredInstant(_:)` recovers the intended day from a legacy instant **without knowing which timezone wrote it** (a stored midnight is within ±12h of that day's UTC midnight, so shifting 12h and reading the day in UTC lands back on it for every offset UTC-12…UTC+12).
- `DayKeyRepairService` runs **first** in `AppStartup` (before the first `await`, because the Today screen loads in parallel): it backfills keys, re-anchors each `date` to local midnight of its key, **merges duplicate pages** for a day (the page with the most completed tasks survives; completed and hand-typed tasks are moved off the loser first, template leftovers are not), and **locks every page whose day is past**. It is a no-op once everything lines up, so it also self-heals on the next timezone change.
- `dayKey` is `Int?` only so an existing store migrates without a rebuild; the repair backfills it. Both repositories keep a legacy fallback that finds an unkeyed row and adopts its key.
- Past-page protection is now **two independent checks** — the `isPastLocked` flag *and* the day key — because either alone has failed: the flag was historically never set at rollover, and the date arithmetic breaks when the clock or timezone moves.
...
```
A day is complete when it is **today or in the past** AND every task is checked off — and an **empty list counts as complete** ("nothing to do = done"), so an empty today/past page is complete. A **future day is NEVER complete**, even if it has tasks and they're all checked off: you can tick tasks ahead of time, but it isn't flagged done and doesn't feed streaks/stats until that day actually arrives (an empty future page that rolls into today flips to complete automatically on the next refresh). Exercise is not in the task list (unless the user separately created a recurring task for it). Calendar-sourced tasks ARE included. The "today vs future" gate lives in `CompletionService.isComplete(tasks:date:today:)`; `recalculate(page:today:)` is called by every mutator and by `getOrCreate`/refresh (which pass the page-generation `today`).
- **`dayComplete` is a CACHE, and it goes stale on past days.** A day that passed while the app never opened it still carries the flag it was given as a FUTURE day (`false`) — so an EMPTY past day stayed grey forever instead of counting as done, and quietly broke the streak. `DailyPageRepository.recalculatePastCompletion(today:)` re-derives the flag for all past pages (called from `AppStartup`), and `getOrCreate` re-derives it for a single past page when you open it, so a stale day fixes itself on view. This is a deliberate, narrow exception to "past pages are never modified automatically": it writes ONLY the derived flag — never a task, note, order, schedule block or lock state — so it can do nothing except bring the flag in line with the tasks already on the page. Do NOT "fix" it as a snapshot-rule violation. [empty-day-complete] *(Owner-approved 2026-06-06, reversing the old "empty = not complete, future days completable" rule.)*
...
**In scope:** Today tasks, Backlog (items + projects + move + multi-select), Recurring, Reminders, Schedule, Exercise, Routines — create/edit/delete/toggle/reorder. **Calendar-sourced Today tasks are ALSO in scope (owner-approved 2026-07-12, reversing the old exclusion):** renaming, editing the note, toggling completion, and deleting a calendar task on Today are all recorded, as is reverting one from the differences page. A calendar rename/note is stored as a per-day **local override** on `CalendarEventLocalState` (`titleOverride`/`notesOverride`) so it survives the next calendar re-sync — the real EKEvent is NEVER mutated. A calendar-task snapshot uses `TaskSnapshot`; the event's hidden/override state uses the new `CalendarStateSnapshot`, and a title/note edit or delete records BOTH. **Still out of scope (never recorded):** EventKit events themselves, calendar SOURCE selection and other calendar settings, the Calendar-tab reconciliation page's restores, all Settings categories (customization/format/accessibility/security/import/export/reset/restore), view switches, sort-mode changes, past-page lock/unlock, and any automatic/system refresh.
