...
- **No filler/explanatory text.** Don't add descriptive or instructional copy anywhere in the app unless it's a message the user genuinely needs (e.g. a destructive-action warning, a real error, a format requirement). If a screen works without the sentence, leave it out — empty states should be blank, not captioned. Prefer no "—" placeholders for empty values either (just leave it blank), except where a value row reads better as "None".
