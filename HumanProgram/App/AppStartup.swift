...
public struct AppStartup {
    public static func run(context: ModelContext, appState: AppState) async throws {
        // 0. FIRST, before anything reads or creates a page: re-file every per-day
        //    record onto its timezone-independent day key, merge any duplicates a
        //    previous timezone change produced, and lock days that are now past.
        //    This must run ahead of the first `await` below — the Today screen starts
        //    loading in parallel, and a page lookup that misses would generate an empty
        //    duplicate over a real day. [tz-daykey]
        _ = try? DayKeyRepairService.run(context: context)

        // 1. Reschedule all notification reminders (rolling 20-occurrence window)
        ...
        // 2. Day-rollover reconciliation of overdue backlog assignments. Read which
        ...
        // 2b. Sever past page-tasks from the backlog/calendar (day-rollover snapshot).
        try pageRepo.severPastTasks(today: today)

        // 2c. Bring stale completion flags on past days up to date (derived value only).
        //     A day that passed while the app never opened it kept the flag it was given
        //     as a FUTURE day — so an empty day stayed grey instead of counting as done.
        //     [empty-day-complete]
        try pageRepo.recalculatePastCompletion(today: today)

        // 3. Ensure every weekday has an exercise routine (creates missing ones)
        try exerciseRepo.ensureSevenWeekdayRoutines()

        // 3b. Fold any old itemized routines into their markdown body (one-time).
        try RoutineRepository(context: context).migrateItemsToBodyIfNeeded()

        // 4. Fetch template inputs
        let inputs = try TemplateInputs.fetchAll(context: context)

        // 5. Ensure today's page exists (created for its side effect)
        ...
        // 6. Refresh today and future pages
        ...
        // 8. Publish today's summary for the home-screen widgets.
